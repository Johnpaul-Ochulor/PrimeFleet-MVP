...
export const uploadPaymentProof = async (req, res) => {
  try {
    // 1. We MUST get the bookingId from the passenger
    const { bookingId, amount, depositAmount, balanceDue, transactionRef, type } = req.body;

    if (!req.file) {
      return res.status(400).json({ 
        success: false, 
        message: "Please upload your payment receipt image." 
      });
    }
    ...
    const payment = await Payment.create({
      bookingId, // <--- This links the payment to Tunde's ride!
      amount,
      depositAmount: depositAmount || amount, // Default to full amount if not split
    ...
    // 3. Update the Booking status so Admin knows it's ready to check
...
export const verifyPayment = async (req, res) => {
  try {
    const { id } = req.params; 
    let { status } = req.body; // Expecting 'CONFIRMED' or 'REJECTED'
    ...
    // 1. Force UpperCase and 2. Map 'SUCCESS' to 'CONFIRMED' if the frontend sends it
    const finalStatus = status.toUpperCase() === 'SUCCESS' ? 'CONFIRMED' : status.toUpperCase();

    payment.status = finalStatus;
    await payment.save();

    // 3. Optional: If payment is CONFIRMED, you might want to auto-update the Booking status too
    // if (finalStatus === 'CONFIRMED' && payment.BookingId) {
    //    await Booking.update({ status: 'PAID' }, { where: { id: payment.BookingId } });
    // }

    res.status(200).json({
      success: true,
      message: `Payment record updated to ${finalStatus}`,
      data: payment
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
